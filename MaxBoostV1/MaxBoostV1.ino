...
float MaxBoostValueLimit                    = 0.99; // maximum value allowed (will be 1.50)
...
void loop() {
  ...
  for(int y=0;y<100;y++) {   // Correr aqui dentro para evitar fazer td constantemente
    ...
    if(RPMSensorValue < 580) {                            // Below 1000 RPM
      if(RPMClimbing && MAPSensorValue > 0.01) {
        lowerDutyCycle();  //is this a good idea???
        Serial.println("Below 1000 RPMClimbing MAP>0.01");
        continue;
      } else {
        stopDutyCycle();
        Serial.println("Below 1000 !RPMClimbing MAP<=0.01");
        ...
        if(RPMClimbing && MAPSensorValue >= (MaxBoostValue - boostTolerance)) {
          ...
          if(!RPMClimbing) {
            ...
            if((previousRPMSensorValue - RPMSensorValue) < RPMChangeUnderLoadPerCycleTolerance)
              ...
              Serial.println("Between 1000 and 2400 !RPMClimbing MAP Under Tolerance");
              continue;
            if((previousRPMSensorValue - RPMSensorValue) > RPMChangeUnderLoadPerCycleTolerance) {
              // Let go of the acelerator 
              Serial.println("Between 1000 and 2400 !RPMClimbing Going down fast");
      ...
      while(dutyCycle < maxDutyCycle && MAPSensorValue < MaxBoostValue) {
        ...
        increaseDutyCycle();
        Serial.println("Increase DutyCycle");
      }
...
void checkDutyCycleSteps() {
  if((MAPSensorValue > 0.10 && MAPSensorValue < MaxBoostValue) || (RPMSensorValue > 580 && RPMClimbing)) {
    dutyCycleSteps = boostDifferencial(); // 1 to 15, depending on map to max diference
  } else {
    // ???    
    dutyCycleSteps = 1;
  }
